...
1. 1아이디와 비밀번호가 서버의 데이터베이스와 일치할 경우 로그인이 되며, 일치하지 않을 경우 로그인이 거절된다.로그인 버튼을 누를 시 바로 메인 화면으로 이동한다.
2. 2회원가입 버튼을 누를 시 회원가입 화면으로 이동한다.
...
1. 1회원가입 화면에서는 사용자의 개인정보를 데이터베이스에 저장하도록 하는 동작을 한다.
2. 2중복확인 버튼 클릭 시 데이터베이스에 동일 아이디나 닉네임이 있는지 확인한다.
...
로그인 후 채팅 페이지로 접속한 결과


로그인 후 채팅 페이지로 접속한 결과

1. 1접속한 사용자의 닉네임이 보인다.
2. 2처음에 접속한 사용자는 대기실이라는 방에 입장한다.
3. 3누군가가 방에 들어오거나 나갈 경우 서버 메시지를 보여준다.
4. 4현재 사용자 목록이 보이며 사용자가 현재 어디 있는 지까지 볼 수 있다.
...
1. 1방을 만들면 방 목록이 갱신되며 방을 만든 사람은 자동으로 그 방으로 입장된다.

방 정보에는 방 번호, 방 이름, 방에 들어간 사람의 수가 보이게 된다.

1. 2사용자가 방을 이동하면 사용자의 위치도 갱신된다.
2. 3방에 입장하면 현재 방 번호와 방제목이 갱신된다.
3. 4사용자가 방에 입장했다는 메시지가 뜬다.
...
1. **8.** 대기실에 있는 사용자와 방 1번 사용자간의 **귓속말**
...
1. 2파일 업로드 시 업로드 진행 상황을 막대 게이지로 표시한다.
...
1. 2사용자가 욕설을 했을 때 (현재 욕설 단어는 &#39;심한욕&#39; 만 등록되어있음)



사진처럼 사용자가 욕설을 하면 채팅 창에는 \*로 필터링 처리되며, 서버 로그에도 기록을 한다.

1. **추가 기능** - 자바스크립트 코드 전송하기(Snippet 기능)
...
1. **2** 코드를 코드 전송 기능을 이용해서 보냈을 때:


코드를 볼 수 있는 창이 하나 생성 되었고 tab과 공백, 색, 글꼴이 구분되어 있어서 코드를 보기 편하다.

1. **3** 메시지 창으로 전송된 메시지를 복사해서 Word 에 붙여 넣으면 그대로 색이 옮겨온다.
